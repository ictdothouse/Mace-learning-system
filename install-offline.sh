...
# ==========================================
# MACE LEARNING SYSTEM - UBUNTU INSTALLATION WIZARD (OFFLINE/WINSCP MODE)
# ==========================================
...
# 5. Mengekstrak fail MACE-Offline.tar.gz yang di-upload
# ==========================================
...
echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}  WIZARD PEMASANGAN VPS MACE (OFFLINE MODE)      ${NC}"
echo -e "${BLUE}=================================================${NC}"

# Pastikan script dijalankan sebagai root
if [ "$EUID" -ne 0 ]; then 
  echo -e "${RED}Sila jalankan script ini sebagai root (Gunakan: sudo bash install-offline.sh)${NC}"
  exit
fi

# Semak jika fail MACE-Offline-Lite.tar.gz wujud
if [ ! -f "MACE-Offline-Lite.tar.gz" ]; then
    echo -e "${RED}RALAT: Fail 'MACE-Offline-Lite.tar.gz' tidak dijumpai di folder ini!${NC}"
    echo -e "${YELLOW}Pastikan anda telah upload fail tersebut menggunakan WinSCP ke folder yang sama dengan script ini.${NC}"
    exit 1
fi
...
echo -e "\n${YELLOW}Langkah 5: Mengekstrak Kod Sistem (MACE-Offline-Lite.tar.gz)...${NC}"
mkdir -p MACE-SYSTEM-DEPLOY
tar -xzf MACE-Offline-Lite.tar.gz -C MACE-SYSTEM-DEPLOY
...
echo -e "\n${YELLOW}Langkah 7: Memasang NPM Dependencies (Backend sahaja)...${NC}"
npm install

echo -e "${BLUE}Melewati proses 'npm run build' kerana folder client/dist sudah wujud dari fail ZIP.${NC}"
